...
from distances import (
    compute_euclidean_dist_no_loops, compute_euclidean_dist_one_loop,
    compute_euclidean_dist_two_loops, compute_mahalanobis_dist, get_sigma
)
...
class KNearestNeighbor(object):
    ...
    def predict(self, X, k=1, distance='euclidean', num_loops=0, sigma_method=None):
        ...
        if distance == 'euclidean':
            if num_loops == 0:
                ...
                dists = compute_euclidean_dist_two_loops(self.X_train, X)
            else:
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # label.                                                                #
            #########################################################################
            y_pred[i] = np.argsort(-np.bincount(closest_y))[0]
            #########################################################################
            #                           END OF YOUR CODE                            #
            #########################################################################
        return y_pred
